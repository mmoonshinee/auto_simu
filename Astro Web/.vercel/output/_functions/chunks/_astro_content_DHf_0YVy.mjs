...
import { J as renderElement, D as removeBase, y as isRemotePath, A as AstroError, L as LiveContentConfigError, a as AstroUserError, U as UnknownContentCollectionError, m as RenderUndefinedEntryError, X as unescapeHTML, P as renderTemplate, C as prependForwardSlash, q as createHeadAndContent, H as renderComponent } from './entrypoint_DrcobDen.mjs';
...
class ImmutableDataStore {
  ...
  static async fromModule() {
    try {
      const data = await import('./_astro_data-layer-content_ugDc9Ac3.mjs');
...
function createGetCollection({
  ...
  return async function getCollection(collection, filter) {
    ...
    if (store.hasCollection(collection)) {
      const { default: imageAssetMap } = await import('./content-assets_BHNbHHLM.mjs');
      const result = [];
...
async function updateImageReferencesInBody(html, fileName) {
  ...
  const { getImage } = await import('./_virtual_astro_get-image_DsEljOmi.mjs');
...
async function renderEntry(entry) {
  ...
  if (entry.deferredRender) {
    try {
      const { default: contentModules } = await import('./content-modules_D42Z1pah.mjs');
...
export { CONTENT_LAYER_TYPE as C, LIVE_CONTENT_TYPE as L, getEntry as a, defineCollection as d, getCollection as g, renderEntry as r };
